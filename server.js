...
// Optional: set COOKIES env var to bypass bot detection on datacenter IPs.
let cookiesFile = '';
if (process.env.COOKIES) {
  try {
    cookiesFile = path.join(os.tmpdir(), 'yt-cookies.txt');
    fs.writeFileSync(cookiesFile, process.env.COOKIES);
  } catch {}
}

function tryExtract(url, playerClient) {
  return new Promise((resolve, reject) => {
    const args = ['-J', '--no-playlist', '--no-warnings', '--extractor-args', `youtube:player_client=${playerClient}`];
...
async function parseYtDlp(url) {
  // iOS returns muxed MP4 and bypasses most bot detection.
  // Fallback to web client if iOS fails.
  try { return await tryExtract(url, 'ios,web'); }
  catch (e1) {
    try { return await tryExtract(url, 'web'); }
