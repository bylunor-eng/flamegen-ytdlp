...
const server = http.createServer(async (req, res) => {
  ...
  if (u.pathname === '/proxy') {
    ...
    if (!targetUrl || !targetUrl.includes('googlevideo.com')) { res.statusCode = 400; return res.end('Invalid url'); }
    try {
      const r = await fetch(targetUrl, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'Range': 'bytes=0-' } });
      ...
      return res.end();
    } catch (e) { res.statusCode = 500; return res.end(JSON.stringify({ error: e.message || 'proxy failed' })); }
  }
  ...
  try {
    ...
    const hdVideo = direct.filter((f) => f.vcodec && f.vcodec !== 'none' && (!f.acodec || f.acodec === 'none') && /mp4/i.test(f.ext)).sort((a, b) => (b.height || 0) - (a.height || 0));
    const hd = hdVideo[0];
    const audioOnly = direct.filter((f) => (!f.vcodec || f.vcodec === 'none') && f.acodec && f.acodec !== 'none' && /m4a/i.test(f.ext)).sort((a, b) => (b.abr || 0) - (a.abr || 0));
    const muxedFormats = direct.filter((f) => f.vcodec && f.vcodec !== 'none' && f.acodec && f.acodec !== 'none' && /mp4/i.test(f.ext)).sort((a, b) => Math.abs((a.height || 0) - 360) - Math.abs((b.height || 0) - 360));
    ...
    res.end(JSON.stringify({ title: info.title || '', thumbnail: info.thumbnail || '', duration: info.duration || 0, medias: [ hd ? { url: proxy(hd.url), quality: `${hd.height || 720}p`, extension: 'mp4', requiresMerge: true, videoAvailable: true, audioAvailable: false } : null, audioOnly ? { url: proxy(audioOnly.url), quality: `${audioOnly.abr || 128}kbps`, extension: 'm4a', requiresMerge: false, videoAvailable: false, audioAvailable: true } : null, muxed ? { url: proxy(muxed.url), quality: `${muxed.height || 360}p`, extension: 'mp4', requiresMerge: false, videoAvailable: true, audioAvailable: true } : null ].filter(Boolean) }));
